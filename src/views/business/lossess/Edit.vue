  ...
  <div class="app-container">
    <div class="top-form">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="盘亏类型" prop="fvehiclenum">
          <el-select v-model="form.fvehiclenum" placeholder="请选择盘亏类型">
...
export default {
  ...
  created() {
    ...
    if(this.keyid) {
      this.$route.meta.title = this.title = '编辑盘亏单'
      this.getOrderDetail(this.keyid)
    }else {
      this.$route.meta.title = this.title = '添加盘亏单'
      this.getBillNumber();
    }
    this.getDictList();
  ...
  watch: {
    $route: {
      handler: function(val){
        if(val.path != '/business/lossess/edit') return
        this.keyid = this.$route.query.keyid
        // this.keyid = this.$route.params.keyid
        if(this.keyid) {
          this.$route.meta.title = this.title = '编辑盘亏单'
