  ...
  <div class="app-container">
    <div class="top-form">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="销售订单" prop="flogisticsnumber">
          <el-select v-model="form.flogisticsnumber" placeholder="请选择销售订单"  @change="handleSelectFCode" clearable filterable remote :remote-method="getSupplierListByCode">
...
export default {
  ...
  created() {
    ...
    if(this.keyid) {
      this.$route.meta.title = this.title = '编辑空瓶出库单'
      this.getOrderDetail(this.keyid)
    }else {
      this.$route.meta.title = this.title = '添加空瓶出库单'
      this.getBillNumber();
    }
    this.getDictList();
  ...
  watch: {
    $route: {
      handler: function(val){
        if(val.path != '/business/bottleTo/edit') return
        this.keyid = this.$route.query.keyid
        if(this.keyid) {
          this.$route.meta.title = this.title = '编辑空瓶出库单'
          this.getOrderDetail(this.keyid);
