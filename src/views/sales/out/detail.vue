    ...
    <div class="top-form">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="计划类型" prop="ft6saletype">
          <el-select v-model="form.ft6saletype" placeholder="请选择计划类型">
        ...
        <el-form-item label="所属部门" prop="fdeliverymethod"> <!-- forgname -->
          <el-select v-model="form.fdeliverymethod" placeholder="请选择所属部门">
            <el-option v-for="(item, index) in deliveryDictList" :key="index" :label="item.fvalue" :value="item.fid" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="下单数量" prop="pubnum">
          <el-input v-model="form.pubnum" type="number" placeholder="请输入下单数量" />
        </el-form-item>
      </el-form>
    </div>
  ...
  watch: {
    $route: {
      handler: function(val){
        if(val.path != '/sales/out/detail') return
        this.keyid = this.$route.query.keyid
        if(this.keyid) {
